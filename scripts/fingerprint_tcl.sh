#!/usr/bin/env bash
# fingerprint_tcl.sh — shared fingerprint module for the Tcl stack.
#
# IDENTICAL algorithm to fingerprint.js / fingerprint.py / fingerprint_bash.sh /
# fingerprint_haskell.sh. Same input+output pair MUST produce the same 7-char hash.
#
...
if [[ -n "${_REGRETS_FINGERPRINT_TCL_SOURCED:-}" ]]; then
  return 0 2>/dev/null || true
fi
_REGRETS_FINGERPRINT_TCL_SOURCED=1
...
if [[ "${1:-}" == "--self-test" ]]; then
  echo "=== fingerprint_tcl.sh self-test ==="
  ...
  [[ $fail_count -eq 0 ]] && echo "✅ ALL PASS — Tcl fingerprint matches JS reference" && exit 0
  exit 1
fi
