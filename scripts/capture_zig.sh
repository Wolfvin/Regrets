...
//
// Compatible with: Zig 0.14+ (verified on 0.16.0).
...
//   - std.os.linux.write/read for stdout/stdin (replaces std.io.getStdOut().writer())
//
...
// ─── stdout/stdin helpers (using linux syscalls — stable across versions) ──

fn writeOut(s: []const u8) void {
    _ = std.os.linux.write(1, s.ptr, s.len);
}

fn writeOutOwned(allocator: std.mem.Allocator, s: []u8) void {
    defer allocator.free(s);
    _ = std.os.linux.write(1, s.ptr, s.len);
}

fn writeOutFmt(allocator: std.mem.Allocator, comptime fmt: []const u8, args: anytype) !void {
...
fn writeErrFmt(allocator: std.mem.Allocator, comptime fmt: []const u8, args: anytype) !void {
    const s = try std.fmt.allocPrint(allocator, fmt, args);
    defer allocator.free(s);
    _ = std.os.linux.write(2, s.ptr, s.len);
}

fn readAllStdin(allocator: std.mem.Allocator, max_size: usize) ![]u8 {
    var buf: std.ArrayList(u8) = .empty;
    errdefer buf.deinit(allocator);
    var chunk: [4096]u8 = undefined;
    while (true) {
        const n = std.os.linux.read(0, &chunk, chunk.len);
        const n_usize: usize = @intCast(n);
        if (n_usize == 0) break;
        if (buf.items.len + n_usize > max_size) return error.StreamTooLong;
        try buf.appendSlice(allocator, chunk[0..n_usize]);
    }
...
pub fn run(comptime UserSource: type) !void {
    ...
    const raw = try readAllStdin(allocator, 16 * 1024 * 1024);
            ...
            if (std.mem.eql(u8, spec.mode, "validate")) {
                ...
                if (expected_hash.len > 0) {
                    if (std.mem.eql(u8, expected_hash, fp)) {
                        writeOut("RESULT PASS\n");
                        pass_count += 1;
                    } else {
                        try writeOutFmt(allocator, "RESULT FAIL hash_mismatch expected={s} actual={s}\n", .{ expected_hash, fp });
                        fail_count += 1;
                    }
                } else {
                    writeOut("RESULT INFO no_expected_hash\n");
                    info_count += 1;
                }
            }
            writeOut("---\n");
    ...
    if (std.mem.eql(u8, spec.mode, "validate")) {
        try writeErrFmt(allocator, "VALIDATE SUMMARY: {} pass, {} fail, {} info\n", .{ pass_count, fail_count, info_count });
