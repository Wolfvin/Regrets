...
async function main() {
  ...
  case 'update': {
    ...
    if (passThroughArgs.includes('--update')) {
      ...
      if (targetStack === 'python' || targetStack === 'php' || targetStack === 'rust' || targetStack === 'go' || targetStack === 'java') {
    ...
    } else if (targetStack === 'java') {
      success = await run('bash', [`${SCRIPTS_DIR}/capture_java.sh`, ...translatedArgs])
    } else {
  ...
  case 'drift': {
    ...
    for (const stack of stacks) {
      if (stack === 'js' || stack === 'ts' || stack === 'react' || stack === 'css') {
        ...
        // Java drift detection: run validate with --runs flag
        const runsArg = passThroughArgs.includes('--runs') ? [] : ['--runs', '5']
        success = await run('bash', [`${SCRIPTS_DIR}/validate_java.sh`, ...runsArg, ...passThroughArgs]) && success
      }
    }
    break
  }
  ...
  case 'ci': {
    ...
    for (const stack of stacks) {
      if (stack === 'js' || stack === 'ts' || stack === 'react' || stack === 'css') {
        ...
        success = await run('bash', [`${SCRIPTS_DIR}/validate_java.sh`, '--fail-fast', ...passThroughArgs]) && success
      }
    }
    break
  }
