...
/**
 * Run a command using spawn (async, non-blocking) so that SIGINT/SIGTERM
 * signals can be forwarded to the child process. This fixes #302 where
 * execFileSync blocked the event loop and prevented signal forwarding.
 *
...
function run(cmd, cmdArgs) {
  ...
  return new Promise((resolve) => {
    ...
    // #302: Forward SIGINT/SIGTERM to the child process.
...
/**
 * Sequential runner: await each run() and accumulate success.
 * Replaces the pattern `success = await run(...) && success` which
 * doesn't work with Promises.
 *
 * @param {Array<[string, string[]]>} commands - Array of [cmd, args] pairs
 * @returns {Promise<boolean>} true if ALL commands succeed
 */
  ...
  try {
    ...
    if (manifest.preBuild) {
      ...
      try {
        ...
        spawnSync(cmd, cmdArgs, { stdio: 'inherit', cwd: process.cwd() })
...
// #302: Wrap dispatch in async IIFE because run() now returns Promises.
;(async () => {
switch (command) {
  case 'install': {
...
ANALYSIS:
  ...
  regret history <clusterId> [--json]                Audit log of contract updates
