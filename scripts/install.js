...
//
// When install auto-generates [null, {}] as inputs, functions often produce
// identical output for all inputs (e.g., add(null, undefined) → NaN,
// add({}, undefined) → NaN). This makes fingerprints useless — every function
// with the same trivial output gets the same hash, so validate always passes
// even when the function changes completely. This guard probes each cluster
// with its auto-generated inputs and skips clusters whose outputs are all
// identical.
 ...
 * Returns { trivial: true, sampleOutput } if all outputs are identical,
 * or { trivial: false } if outputs vary (or if probing is not possible).
 *
...
async function probeTrivialOutputs(cluster) {
  ...
  for (const input of cluster.inputs) {
    try {
      ...
      // Function threw — record as a special marker
      outputs.push('__PROBE_THREW__')
    }
  }

  // Need at least 2 outputs to compare
  if (outputs.length < 2) return { trivial: false }

  const serialized = outputs.map(serializeOutput)
  const first = serialized[0]
  const allIdentical = serialized.every(s => s === first)

  if (allIdentical) {
    return { trivial: true, sampleOutput: outputs[0] }
  }
...
async function main() {
  ...
  // Probe new clusters with auto-generated inputs. If all outputs are
  // identical, the fingerprint would be useless — skip those clusters
  ...
  if (newClusters.length > 0 && !dryRun) {
    ...
    for (const { cluster, probe } of probeResults) {
      if (probe.trivial) {
        ...
        const outputLabel = probe.sampleOutput === null ? 'null'
          : probe.sampleOutput === undefined ? 'undefined'
          : Number.isNaN(probe.sampleOutput) ? 'NaN'
          : String(probe.sampleOutput)
        process.stderr.write(
          `⚠️  Cluster "${cluster.entry}" skipped — auto-generated inputs all produce identical output (${outputLabel}). Add meaningful inputs manually in regrets/manifest.json.\n`
        )
      } else {
        kept.push(cluster)
      }
  ...
  if (trivialSkipped > 0) {
    console.log(`${trivialSkipped} skipped due to trivial auto-generated inputs (all outputs identical)`)
  }
