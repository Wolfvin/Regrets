...
# caused a real false-negative: capture/validate reported PASS on a target
# module whose behavior had actually changed. `dont_write_bytecode` only
# stops new .pyc files from being written in THIS process — it does NOT stop
# Python from reading an existing stale .pyc left on disk by a prior process,
# so we also proactively delete any __pycache__ directories before importing
