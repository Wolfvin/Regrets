...
//
// Multi-input support (#315 parity): capture processes ALL inputs from
// the manifest's inputs[] array. The first input is stored as the
// top-level INPUT/OUTPUT/HASH lines. Additional inputs are stored in
// an INPUTS line (JSON array of {input, output, hash} for inputs[1+]).
// Validate compares ALL input hashes — any mismatch FAILs the cluster.
//
...
static char* compute_fingerprint(json_object* input, json_object* output) {
    ...
    // SHA-256 = 256 bits → base36 needs at most ceil(256/log2(36)) ≈ 50 chars
    char tmp[128];
    int pos = 0;
    if (BN_is_zero(bn)) {
        tmp[pos++] = '0';
...
typedef struct {
    json_object* input;    // owned reference
    json_object* output;   // owned reference
    char* fingerprint;     // malloc'd 7-char string
    int skipped;           // trivial-input guard
} input_result_t;

static void free_input_result(input_result_t* r) {
    if (r->input) json_object_put(r->input);
    if (r->output) json_object_put(r->output);
    free(r->fingerprint);
}
...
static char* build_regret_content(json_object* cluster, const char* id,
                                  ...
                                  input_result_t* results, int n_results) {
    char* buf = NULL;
    size_t len = 0;
    FILE* mem = open_memstream(&buf, &len);
    if (!mem) return NULL;

    // First valid result for top-level fields
    input_result_t* primary = NULL;
    for (int i = 0; i < n_results; i++) {
        if (!results[i].skipped) { primary = &results[i]; break; }
    }
    if (!primary) {
        // All skipped — shouldn't reach here but handle gracefully
        fclose(mem);
        free(buf);
        return NULL;
    }
    ...
    // Top-level INPUT/OUTPUT/HASH from first valid result
    const char* input_str = json_object_to_json_string_ext(primary->input, JSON_C_TO_STRING_PLAIN);
    const char* output_str = json_object_to_json_string_ext(primary->output, JSON_C_TO_STRING_PLAIN);
    fprintf(mem, "INPUT  %s\n", input_str);
    fprintf(mem, "OUTPUT %s\n", output_str);
    fprintf(mem, "HASH   %s\n", primary->fingerprint);

    // Issue #315: per-input contracts for inputs[1+].
    // Only emit INPUTS line when there are additional valid results beyond the first.
    // Backward-compatible: old .regret files (no INPUTS line) still work.
    int extra_count = 0;
    int primary_idx = -1;
    for (int i = 0; i < n_results; i++) {
        if (!results[i].skipped) {
            if (primary_idx < 0) primary_idx = i;
            else extra_count++;
        }
    }
    ...
    if (extra_count > 0) {
        ...
        int first = 1;
        for (int i = 0; i < n_results; i++) {
            if (results[i].skipped || i == primary_idx) continue;
            if (!first) fprintf(mem, ",");
            first = 0;
            const char* ri = json_object_to_json_string_ext(results[i].input, JSON_C_TO_STRING_PLAIN);
            const char* ro = json_object_to_json_string_ext(results[i].output, JSON_C_TO_STRING_PLAIN);
            fprintf(mem, "{\"input\":%s,\"output\":%s,\"hash\":\"%s\"}", ri, ro, results[i].fingerprint);
        }
        fprintf(mem, "]\n");
    }
...
typedef struct {
    ...
    char hash[64];
    char inputs_line[65536];  // INPUTS line for multi-input (#315)
} parsed_regret_t;
...
static int parse_regret(const char* content, parsed_regret_t* out) {
    ...
    while (*p) {
        ...
        } else if (key_len == 6 && strncmp(p, "INPUTS", 6) == 0) {
            // INPUTS line — multi-input (#315)
            if (val_len < sizeof(out->inputs_line)) {
                memcpy(out->inputs_line, val_start, val_len);
                out->inputs_line[val_len] = '\0';
            }
...
// ─── Invoke entry for a single input ──────────────────────────────────────

static input_result_t invoke_entry(regret_entry_fn fn, json_object* raw_input) {
    input_result_t r = {0};

    // Own a reference to the input
    r.input = raw_input;
    json_object_get(r.input);

    const char* input_str = json_object_to_json_string_ext(r.input, JSON_C_TO_STRING_PLAIN);
    ...
    r.output = json_tokener_parse(output_str);
    free(output_str);
    if (!r.output) {
    ...
    if (json_object_is_type(r.output, json_type_null)) {
        // Null output — trivial-input guard
        r.skipped = 1;
        json_object_put(r.output);
        r.output = NULL;
        return r;
    }

    r.fingerprint = compute_fingerprint(r.input, r.output);
    if (!r.fingerprint) {
        r.skipped = 1;
        json_object_put(r.output);
        r.output = NULL;
...
static int run_capture(const char* manifest_path, const char* cluster_filter) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        // Get all inputs from inputs[]
        struct json_object* inputs_arr = NULL;
        json_object_object_get_ex(ci->cluster_obj, "inputs", &inputs_arr);
        int n_inputs = 0;
        if (inputs_arr && json_object_is_type(inputs_arr, json_type_array)) {
            n_inputs = json_object_array_length(inputs_arr);
        }

        if (n_inputs == 0) {
            // No inputs defined — try with null
            input_result_t result = invoke_entry(fn, json_object_new_null());
            if (result.skipped) {
                printf("   ⏭️  Skipped: trivial-input guard (null input)\n");
                json_object_put(result.input);
                skipped++;
            } else {
                // Write single-result .regret
        ...
        // Process ALL inputs (#315 parity)
        input_result_t* results = calloc(n_inputs, sizeof(input_result_t));
        int n_valid = 0;
        for (int j = 0; j < n_inputs; j++) {
            json_object* raw_input = json_object_array_get_idx(inputs_arr, j);
            results[j] = invoke_entry(fn, raw_input);
            if (!results[j].skipped) n_valid++;
        }

        if (n_valid == 0) {
            printf("   ⏭️  Skipped: all inputs returned trivial output\n");
            for (int j = 0; j < n_inputs; j++) free_input_result(&results[j]);
            free(results);
            skipped++;
            continue;
        }

        // Build .regret content with all valid results
        char* regret_content = build_regret_content(ci->cluster_obj, ci->id,
                                                    ci->entry, results, n_inputs);
        if (!regret_content) {
            printf("   ❌ Failed to build .regret content\n");
            for (int j = 0; j < n_inputs; j++) free_input_result(&results[j]);
            free(results);
        ...
        // Print summary for first valid result
        for (int j = 0; j < n_inputs; j++) {
            if (!results[j].skipped) {
                printf("   ✅ Fingerprint: %s\n", results[j].fingerprint);
                break;
            }
        }
        if (n_valid > 1) {
            printf("   📊 %d inputs captured (%d valid)\n", n_inputs, n_valid);
        }
        printf("   📄 Saved: %s\n", regret_path);
...
static int run_validate(const char* manifest_path, const char* cluster_filter) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        // ── Validate first input (top-level INPUT/HASH) ──
        ...
        int cluster_failed = 0;

        if (live_fp && strcmp(parsed.hash, live_fp) == 0) {
            // Top-level hash matches — check INPUTS line too (#315)
        } else {
        ...
        // ── Validate INPUTS line (multi-input #315) ──
        if (!cluster_failed && parsed.inputs_line[0]) {
            // Parse the INPUTS JSON array
            json_object* inputs_arr = json_tokener_parse(parsed.inputs_line);
            if (inputs_arr && json_object_is_type(inputs_arr, json_type_array)) {
                int n_extra = json_object_array_length(inputs_arr);
                for (int j = 0; j < n_extra; j++) {
                    json_object* entry_obj = json_object_array_get_idx(inputs_arr, j);
                    struct json_object* e_input = NULL;
                    struct json_object* e_hash = NULL;

                    if (!json_object_object_get_ex(entry_obj, "input", &e_input)) continue;
                    if (!json_object_object_get_ex(entry_obj, "hash", &e_hash)) continue;

                    const char* golden_hash = json_object_get_string(e_hash);
                    const char* e_input_str = json_object_to_json_string_ext(e_input, JSON_C_TO_STRING_PLAIN);

                    char* xlive_output_str = fn(e_input_str);
                    if (!xlive_output_str) {
                        printf("   ❌ FAIL  input[%d] returned NULL on re-invoke\n", j + 1);
                        cluster_failed = 1;
                        continue;
                    }
                    json_object* xlive_output = json_tokener_parse(xlive_output_str);
                    free(xlive_output_str);
                    if (!xlive_output) {
                        printf("   ❌ FAIL  input[%d] returned invalid JSON\n", j + 1);
                        cluster_failed = 1;
                        continue;
                    }

                    char* xlive_fp = compute_fingerprint(e_input, xlive_output);
                    if (!xlive_fp || strcmp(golden_hash, xlive_fp) != 0) {
                        printf("   ❌ FAIL  input[%d] golden=%s live=%s\n",
                               j + 1, golden_hash, xlive_fp ? xlive_fp : "(null)");
                        cluster_failed = 1;
                    }
                    free(xlive_fp);
                    json_object_put(xlive_output);
                }
            }
            if (inputs_arr) json_object_put(inputs_arr);
        }
        ...
        if (cluster_failed) {
            ...
            printf("   ✅ PASS  (hash %s)\n", parsed.hash);
            if (parsed.inputs_line[0]) {
                printf("   📊 All input hashes match (multi-input)\n");
            }
            passed++;
        }
    }
