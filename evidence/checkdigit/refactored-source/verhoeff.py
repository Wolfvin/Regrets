...
# Dihedral group D5 multiplication table (Cayley table)
...
# Inverse table for the dihedral group
INVERSE: TupleType[int] = (0, 4, 3, 2, 1, 5, 6, 7, 8, 9)

# Permutation table (8 permutations cycling)
...
def _dihedral_multiply(a: int, b: int) -> int:
    """Multiply two elements in the dihedral group D5 using the Cayley table.

    Args:
        a: First element (0-9)
        b: Second element (0-9)

    Returns:
        int: Result of D5 multiplication
    """
    return DIHEDRAL_CAYLEY[a][b]


def _permute(position: int, digit: int) -> int:
    """Apply the permutation table for a given position and digit.

    The permutation cycles every 8 positions.

    Args:
        position: 1-based position from the right
        digit: The digit value (0-9)

    Returns:
        int: Permuted digit value
    """
    return PERMUTATION[position % 8][digit]
...
def calculate(data: str) -> str:
    ...
    checksum = 0
    data = cleanse(data)
    # Process digits from right to left, position starts at 1
    for position, value in enumerate(data[::-1], 1):
        permuted = _permute(position, int(value))
        checksum = _dihedral_multiply(checksum, permuted)
