...
def _parity_bit(count_ones: int, even: bool) -> str:
    """Determine the parity bit based on the count of 1-bits.

    Args:
        count_ones: Number of 1-bits in the data
        even: True for even parity, False for odd parity

    Returns:
        str: '0' or '1' representing the parity bit
    """
    is_even_count = count_ones % 2 == 0
    if even:
        return "0" if is_even_count else "1"
    else:
        return "1" if is_even_count else "0"
...
def calculate(data: str, even: bool = True) -> str:
    ...
    """
    data = cleanse(data)
    return _parity_bit(data.count("1"), even)
