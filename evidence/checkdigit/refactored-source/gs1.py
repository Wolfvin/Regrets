...
def calculate(data: str) -> str:
    ...
    """
    data = cleanse(data)
    # Reverse so last digit gets weight 3, second-to-last gets 1, etc.
    reversed_data = data[::-1]
    # Use itertools.cycle for alternating weights (3, 1, 3, 1, ...)
    weights = cycle([3, 1])
    total_sum = sum(int(digit) * next(weights) for digit in reversed_data)
