...
def _xor_divide(bitarray: list, polynomial: str) -> list:
    """Perform binary XOR long division on a bitarray with the given polynomial.

    This implements the core CRC division algorithm: repeatedly XOR the
    leading bits of the bitarray with the polynomial, then strip leading zeros.

    Args:
        bitarray: List of single-character binary digits ('0' or '1')
        polynomial: Binary string representing the divisor polynomial

    Returns:
        list: Remaining bitarray after division (the CRC remainder)
    """
    while len(bitarray) >= len(polynomial):
        # XOR current leading segment with polynomial
        for position, bit in enumerate(polynomial):
            bitarray[position] = "0" if bit == bitarray[position] else "1"
        # Strip leading zeros
        while bitarray and bitarray[0] == "0" and len(bitarray) >= len(polynomial):
            bitarray.pop(0)
    return bitarray
...
def calculate(data: str, polynomial: str, pad: str = "0") -> str:
    ...
    bitarray = list(data)
    remainder = _xor_divide(bitarray, polynomial)
...
def validate(data: str, polynomial: str) -> bool:
    ...
    """
    data = cleanse(data)
    # The principle of CRCs is that when done again with the check digit
