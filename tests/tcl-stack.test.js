// tests/tcl-stack.test.js — end-to-end test for the Tcl stack
//
// Runs scripts/capture_tcl.sh and scripts/validate_tcl.sh against the
// tests/fixtures/tcl-example fixture, then asserts:
...
//   4. cross-stack parity: Tcl-written HASH matches JS fingerprint()
//   5. JS validate.js can parse Tcl-generated .regret (cross-tool compat)
//
// Skips automatically if `tclsh` is not on PATH.
//
// Run: node --test tests/tcl-stack.test.js
...
const CAPTURE_TCL = join(SCRIPTS_DIR, 'capture_tcl.sh')
const VALIDATE_TCL = join(SCRIPTS_DIR, 'validate_tcl.sh')
const FIXTURE = join(ROOT, 'tests', 'fixtures', 'tcl-example')

// ─── Skip if `tclsh` is not available ────────────────────────────────────────

function tclAvailable() {
  // Try standard tclsh, then tclsh8.6, then the locally-built one
  for (const bin of ['tclsh', 'tclsh8.6', '/tmp/tcl-install/usr/local/bin/tclsh8.6']) {
    const r = spawnSync(bin, ['-c', 'puts ok'], {
      encoding: 'utf8', timeout: 5_000,
      env: { ...process.env, LD_LIBRARY_PATH: `/tmp/tcl-install/usr/local/lib:${process.env.LD_LIBRARY_PATH || ''}` },
    })
...
const hasTcl = tclAvailable()

function getTclEnv() {
  return {
    ...process.env,
    PATH: `/tmp/tcl-install/usr/local/bin:${process.env.PATH}`,
    LD_LIBRARY_PATH: `/tmp/tcl-install/usr/local/lib:${process.env.LD_LIBRARY_PATH || ''}`,
  }
}

function runBash(scriptPath, args = [], cwd = FIXTURE) {
  const result = spawnSync('bash', [scriptPath, ...args], {
    cwd, encoding: 'utf8', timeout: 60_000, env: getTclEnv(),
  })
  return { exitCode: result.status ?? 1, stdout: result.stdout ?? '', stderr: result.stderr ?? '' }
}

describe('Tcl stack — capture + validate', { skip: !hasTcl && 'tclsh not on PATH' }, () => {
  beforeEach(() => {
  ...
  it('capture writes .regret files with all standard fields + INPUTS line for multi-input', () => {
    const result = runBash(CAPTURE_TCL)
    ...
    for (const id of ['slugify-fn', 'count-vowels-fn', 'reverse-fn', 'add-fn']) {
      ...
      assert.match(content, /^stack: tcl/m, `${id}: missing/wrong stack header`)
  ...
  it('validate (no code change) exits 0 and prints PASS for all clusters', () => {
    runBash(CAPTURE_TCL)
    const result = runBash(VALIDATE_TCL)
    assert.equal(result.exitCode, 0,
      `validate failed\nstdout: ${result.stdout}\nstderr: ${result.stderr}`)
  ...
  it('validate detects breaking change → exit 1, FAIL', () => {
    runBash(CAPTURE_TCL)
    const srcPath = join(FIXTURE, 'stringutils.tcl')
    const original = readFileSync(srcPath, 'utf8')
    // Mutate reverse_str to use toupper instead of reverse
    const mutated = original.replace('string reverse $s', 'string toupper $s')
  ...
  it('cross-stack parity: Tcl-written HASH matches JS fingerprint()', () => {
    runBash(CAPTURE_TCL)
    for (const id of ['slugify-fn', 'count-vowels-fn', 'reverse-fn', 'add-fn']) {
      const regretPath = join(FIXTURE, 'regrets', `${id}.regret`)
      const content = readFileSync(regretPath, 'utf8')
      ...
      assert.equal(jsHash, regret.goldenHash,
        `${id}: cross-stack parity FAILED — JS computed "${jsHash}" but Tcl .regret stored "${regret.goldenHash}"`)
  ...
  it('JS validate.js can parse Tcl-generated .regret (cross-tool compat)', () => {
    runBash(CAPTURE_TCL)
    const content = readFileSync(join(FIXTURE, 'regrets', 'add-fn.regret'), 'utf8')
    const regret = parseRegret(content)
    assert.equal(regret.stack, 'tcl')
