...
/**
 * Parse classic context-sensitive production syntax.
 * Transforms patterns like 'A<B>C' → predecessor 'B' with leftCtx 'A' and rightCtx 'C'.
 * If no context markers (< or >) are present, returns the production unchanged.
 *
 * @param {Array} production - [pattern, successor] pair
 * @returns {Array} - [predecessor, productionObject] pair
 */
export function transformClassicCSProduction(production) {
  const pattern = production[0];
  const successorDef = production[1];

  // Match left context: e.g., 'A<B' → leftCtx='A', predecessor='B'
  const leftMatch = pattern.match(/(.+)<(.)/);
  // Match right context: e.g., 'B>C' → predecessor='B', rightCtx='C'
  const rightMatch = pattern.match(/(.)>(.+)/);

  // No context markers found — return original production unchanged
  if (!leftMatch && !rightMatch) {
    return production;
  }

  // Build production object from existing or wrap successor
  const hasStructuredSuccessor = (successorDef.successor || successorDef.successors);
  const productionObject = hasStructuredSuccessor ? successorDef : { successor: successorDef };

  let predecessor;

  if (leftMatch) {
    predecessor = leftMatch[2];
    productionObject.leftCtx = leftMatch[1];
  }

  if (rightMatch) {
    predecessor = rightMatch[1];
    productionObject.rightCtx = rightMatch[2];
  }
