...
/**
 * Extract the symbol character from a part (string or object).
 * Handles both string-based and object-based ({symbol, params}) axioms.
 */
function resolveSymbol(part) {
  return part.symbol || part;
}

/**
 * Extract params from a part if it's an object, empty array otherwise.
 */
...
/**
 * Append a result to the new axiom being built during applyProductions.
 * Handles string concatenation, array spreading, and single-element pushing.
 */
function appendResult(newAxiom, result) {
...
export default class LSystem {
  ...
  /**
   * Check context-sensitive conditions for a production.
   * Returns true if the production's context constraints are satisfied.
   */
  _checkContext(p, index) {
    const hasBoth = p.leftCtx !== undefined && p.rightCtx !== undefined;
    const matchOpts = { branchSymbols: this.branchSymbols, ignoredSymbols: this.ignoredSymbols };

    if (hasBoth) {
      return this.match({ ...matchOpts, direction: 'left', match: p.leftCtx, index }).result
          && this.match({ ...matchOpts, direction: 'right', match: p.rightCtx, index }).result;
    }
    if (p.leftCtx !== undefined) {
      return this.match({ ...matchOpts, direction: 'left', match: p.leftCtx, index }).result;
    }
    if (p.rightCtx !== undefined) {
      return this.match({ ...matchOpts, direction: 'right', match: p.rightCtx, index }).result;
    }
    return true;
  }
  ...
  getProductionResult(p, index, part, params, recursive = false) {
    ...
    if (conditional && p.condition({index, currentAxiom: this.axiom, part, params}) === false) {
      ...
      precheck = this._checkContext(p, index);
    }
  ...
  applyProductions() {
    ...
    for (let part of this.axiom) {
      const symbol = resolveSymbol(part);
      ...
      newAxiom = appendResult(newAxiom, result);
      index++;
    }
