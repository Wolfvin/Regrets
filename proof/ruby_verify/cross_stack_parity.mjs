#!/usr/bin/env node
// cross_stack_parity.mjs — verify the Ruby stack's fingerprint is byte-identical
...
// proof/java_verify/, proof/php_verify/, proof/rust_verify/, proof/go_verify/,
// and proof/c_verify/, so we can verify 7-way parity:
// Ruby == Java == PHP == Rust == Go == C == JS == Python.
...
const tmpDir = mkdtempSync(join(tmpdir(), 'regret-ruby-parity-'))
...
console.log(`Cross-stack parity check for ${regretFiles.length} Ruby clusters`)
console.log('═'.repeat(78))
console.log('cluster                       | Ruby hash | JS hash   | Py hash   | match')
console.log('─'.repeat(78))
...
for (const file of regretFiles.sort()) {
  ...
  const { meta, input, output, hash: rubyHash } = parseRegret(content)

  const jsHash = fingerprint(input, output)
  const pyHash = pyFingerprint(input, output)
  const match = rubyHash === jsHash && jsHash === pyHash
  if (!match) allMatch = false

  const id = meta.cluster.padEnd(29)
  const mark = match ? '✅' : '❌'
  console.log(`${id} | ${rubyHash}   | ${jsHash}   | ${pyHash}   | ${mark}`)
}

console.log('═'.repeat(78))
rmSync(tmpDir, { recursive: true, force: true })
if (allMatch) {
  console.log('✅ All 3 stacks agree — Ruby is byte-identical to JS and Python.')
  console.log('   Bonus: same hashes match Java (proof/java_verify/), PHP (proof/php_verify/),')
  console.log('          Rust (proof/rust_verify/), Go (proof/go_verify/), and C (proof/c_verify/)')
  console.log('          → 7-way cross-stack parity: Ruby == Java == PHP == Rust == Go == C == JS == Python.')
  process.exit(0)
} else {
