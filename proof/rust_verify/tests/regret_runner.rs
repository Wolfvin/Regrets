//! regret_runner.rs — Independent verification runner for the Rust Regrets stack (PR #355).
//!
//! Adapted from references/rust/tests/regret_runner.rs (PR #355) to dispatch
//! to the 5 fresh functions in regret_verify::*. The fingerprint module is
//! reused as-is from the lib — same algorithm, byte-identical output.
//!
...
use regret_verify::fingerprint;
use regret_verify::{slugify, base64_encode, crc32, fnv1a, is_valid_ipv4};
...
fn call_function(entry: &str, input: &Value) -> Value {
    match entry {
        "slugify" => {
            let s = input.as_str().expect("slugify: input must be string");
            json!(slugify(s))
        }
        "base64_encode" => {
            let s = input.as_str().expect("base64_encode: input must be string");
            json!(base64_encode(s))
        }
        "crc32" => {
            let s = input.as_str().expect("crc32: input must be string");
            json!(crc32(s))
        }
        "fnv1a" => {
            let s = input.as_str().expect("fnv1a: input must be string");
            json!(fnv1a(s))
        }
        "is_valid_ipv4" => {
            let s = input.as_str().expect("is_valid_ipv4: input must be string");
            json!(is_valid_ipv4(s))
        }
...
// ─── .regret file format (compatible with JS/Python/Go stacks) ───────────────
