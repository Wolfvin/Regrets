...
## Solution: `freezeTime` in Manifest
...
| `datetime.now()` | Returns frozen `datetime` object |
...
A related improvement: `deep_clone()` now handles `time.struct_time` objects by converting them to a deterministic list `[year, mon, mday, hour, min, sec, wday, yday, isdst]`. Previously, `struct_time` fell through to `repr()`, which was technically deterministic but produced verbose output like `time.struct_time(tm_year=2025, tm_mon=6, ...)` that was harder to work with.
