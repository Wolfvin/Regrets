...
Go stack support is now **working end-to-end** — both `capture_go.sh capture` and `capture_go.sh validate` generate and run real Go test files that invoke entry functions, compute cross-stack-compatible fingerprints, and write/compare `.regret` files. This is based on real-world testing against the `baris-inandi/bfgo` project (a Brainfuck compiler/interpreter in Go).
...
- **Capture**: `bash scripts/capture_go.sh capture` generates `regrets/regret_capture_test.go` + `regrets/regret_helpers_test.go`, runs `go test`, writes `.regret` files for each cluster.
- **Validate**: `bash scripts/capture_go.sh validate` generates `regrets/regret_validate_test.go`, re-invokes each entry function with the saved input, compares the live fingerprint to the golden, reports PASS/FAIL.
...
- **Multi-input INPUTS line (#315 parity)**: currently the `.regret` file stores only the first successful input as the golden. Multi-input contracts would need the `INPUTS` line added to the Go capture path.
